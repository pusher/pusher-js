...
describe("EncryptedChannel", function() {
  ...
  describe("#trigger-encrypted", function() {
    ...
    beforeEach(() => {
      ...
      randomBytesSpy = spyOn(tweetNacl, 'randomBytes').and.returnValue(nonceBytes)

    });
    it("should raise an exception if the event name does not start with client-", function(done) {
      let thenFn = jasmine.createSpy()
      channel.triggerEncrypted("whatever", {})
        .then(thenFn)
        .catch(e =>{
          expect(thenFn).not.toHaveBeenCalled();
          // expect(e).toBe(jasmine.any(Errors.BadEventName))
          // There is some weird bundling stuff causing this e to not be an
          // instanceof the Errors.BadEventName class
          expect(e.message).toBe("Event 'whatever' does not start with 'client-'");
          done();
        });
    });
  ...
  describe("#handleEvent", function() {
    ...
    describe("on other events", function() {
      beforeEach(() => {
        ...
        let randomBytesSpy = spyOn(tweetNacl, 'randomBytes').and.returnValue(nonceBytes)
      });
