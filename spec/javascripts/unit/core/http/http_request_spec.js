...
describe("HTTPRequest", function() {
  ...
  describe("#close", function() {
    ...
    it("should unregister the unloader", function() {
      var unloader = Runtime.addUnloadListener.calls.argsFor(0)[0];
      request.close();
  ...
  describe("on chunk", function() {
    ...
    it("should emit all chunks send in one batch", function() {
      ...
      expect(onChunk.calls.argsFor(0)[0]).toEqual({ status: 200, data: "c1" });
      expect(onChunk.calls.argsFor(1)[0]).toEqual({ status: 200, data: "c2" });
  ...
  describe("on page unload", function() {
    ...
    beforeEach(function() {
      request.start("test payload");
      unloader = Runtime.addUnloadListener.calls.argsFor(0)[0];
    });
